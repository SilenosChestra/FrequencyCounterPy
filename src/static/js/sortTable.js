...
initial_sort_id = 0; // нумар пачатковага адсартыраванага слупка, пачынаючы з нуля
initial_sort_up = 0; // 0 - сартыроўка ўніз, 1 - сартыроўка ўверх
...
function _sort(a, b) {
	...
	else if (!sort_case_sensitive) return sort_insensitive_bel(a, b);
...
function sort_insensitive_bel(a, b) {
	var anew = a.toLowerCase();
	var bnew = b.toLowerCase();
	anew = (anew + '').replace(/[\+=]/g,'');
	bnew = (bnew + '').replace(/[\+=]/g,'');
	anew = (anew + '').replace(/і/g, "и");
	bnew = (bnew + '').replace(/і/g, "и");
	if (anew < bnew) return -1;
	if (anew > bnew) return 1;
	return 0;
}
...
function init(e) {
	...
	for (var j = 0; (thead = document.getElementsByTagName("thead").item(j)); j++) {
		...
		for (var i = 0; (node = thead.getElementsByTagName("td").item(i)); i++) {
			...
			node.title = "Цісніце на загаловак, каб адсартаваць табліцу па гэтым слупку";
		}
